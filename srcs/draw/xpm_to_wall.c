...
/*   Updated: 2023/12/02 15:18:13 by hhino            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_wall_addr(t_texture	*texture)
{
	texture->south->addr = (int *)mlx_get_data_addr(texture->south->wall_img, \
		&texture->south->bpp, &texture->south->line_length, &texture->south->endian);
	texture->north->addr = (int *)mlx_get_data_addr(texture->north->wall_img, \
		&texture->north->bpp, &texture->north->line_length, &texture->north->endian);
	texture->west->addr = (int *)mlx_get_data_addr(texture->west->wall_img, \
		&texture->west->bpp, &texture->west->line_length, &texture->west->endian);
	texture->east->addr = (int *)mlx_get_data_addr(texture->east->wall_img, \
		&texture->east->bpp, &texture->east->line_length, &texture->east->endian);
}
