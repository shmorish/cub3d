...
/*   Updated: 2023/11/11 13:46:19 by morishitash      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	get_ray_y_up(t_data *data, double ray)
{
	...
	while (1)
	{
		...
		check_x = ((check_y - data->player_pos_y) / tan(ray)) \
					+ data->player_pos_x;
		if (check_y < 0 || check_y > data->parser->map_height + 1 || check_x < 1 || check_x > data->parser->map_width + 1)
			return (__DBL_MAX__);
		if (data->parser->map[check_y][(int)(check_x - 1)] == '1')
		{
